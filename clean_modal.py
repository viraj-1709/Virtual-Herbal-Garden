...
export default function PlantDetailModal({ plant, onClose, onOpen3D }) {
  ...
  const handlePlayAudio = () => {
    ...
    const speechText = `${commonName}, botanical name ${plant.scientificName}, family ${plant.family}. ${plant.shortDescription}. Verified uses: ${plant.medicinalUses.join('. ')}`;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl max-h-[92vh] flex flex-col rounded-3xl overflow-hidden glass-panel border border-emerald-500/30 shadow-2xl bg-white dark:bg-herbal-darkCard">
        ...
        <div className="relative h-60 sm:h-72 w-full overflow-hidden flex-shrink-0 bg-slate-900">
          <img
        ...
        <div className="flex items-center gap-2 px-5 py-2.5 border-b border-emerald-500/20 overflow-x-auto select-none bg-emerald-50/50 dark:bg-emerald-950/30">
          {[
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6 text-slate-800 dark:text-slate-200">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {plant.ayushSystems.map((sys) => (
                    <span
                      key={sys}
                      className="px-3 py-1 rounded-xl text-xs font-bold bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800"
                    >
                      {sys}
                    </span>
                  ))}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {plant.medicinalUses.map((use, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-2xl glass-panel border border-emerald-500/20 flex items-start gap-2.5 text-xs font-medium"
                    >
                      <span className="w-4 h-4 rounded-full bg-emerald-500/20 text-emerald-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold">
                        ✓
                      </span>
                      <span>{use}</span>
                    </div>
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="p-5 rounded-2xl glass-panel border border-emerald-500/30 bg-emerald-50/40 dark:bg-emerald-950/20 space-y-2">
                <h4 className="font-heading font-bold text-sm text-emerald-900 dark:text-emerald-200">
                  Classical Literature & Traditional Wisdom
                </h4>
          ...
          {activeTab === 'pharmacology' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-0.5">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Rasa (Taste)</span>
                  <p className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-100">{plant.ayurvedicProfile.rasa}</p>
                </div>
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-0.5">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Guna (Qualities)</span>
                  <p className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-100">{plant.ayurvedicProfile.guna}</p>
                </div>
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-0.5">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Virya (Potency)</span>
                  <p className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-100">{plant.ayurvedicProfile.virya}</p>
                </div>
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-0.5">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Vipaka (Post-Digestive Effect)</span>
                  <p className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-100">{plant.ayurvedicProfile.vipaka}</p>
                </div>
              </div>
          ...
          {activeTab === 'cultivation' && (
            <div className="space-y-4 animate-in fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-1">
                  <span className="text-xs font-bold text-slate-500 flex items-center gap-1.5">
                    <Compass className="w-3.5 h-3.5 text-emerald-600" />
                    Climate & Habitat
                  </span>
                  <p className="text-xs text-slate-800 dark:text-slate-200">{plant.cultivation.climate}</p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-1">
                  ...
                  <p className="text-xs text-slate-800 dark:text-slate-200">{plant.cultivation.soil}</p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-1">
                  ...
                  <p className="text-xs text-slate-800 dark:text-slate-200">{plant.cultivation.sunlight}</p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl glass-panel border border-emerald-500/20 space-y-1">
                  ...
                  <p className="text-xs text-slate-800 dark:text-slate-200">{plant.cultivation.watering}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSaveNotes} className="space-y-4 animate-in fade-in">
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full p-3.5 rounded-2xl glass-panel border border-emerald-500/30 text-xs sm:text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-emerald-500"
                />
              </div>

              <div className="flex items-center justify-between">
                <span className="text-[10px] text-slate-400">
                  Notes are stored locally in your browser and synced with your profile.
                </span>
                <button
                  type="submit"
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-700 transition-all shadow-sm"
                >
                  {savedNoteSuccess ? <Check className="w-3.5 h-3.5" /> : <Save className="w-3.5 h-3.5" />}
...
print("PlantDetailModal.jsx cleaned without emojis.")
